...
async function run_all() {
  hitCountApi();
  if (
    window.location.href.indexOf("screen=place&mode=scavenge") < 0
  ) {
    window.location.assign(
      game_data.link_base_pure + "place&mode=scavenge"
    );
  }
  if (parseFloat(game_data.majorVersion) < 8.177) {
    var scavengeInfo = JSON.parse(
      $("html")
        .find('script:contains("ScavengeScreen")')
        .html()
        .match(/\{.*\:\{.*\:.*\}\}/g)[0]
    );
    ...
    duration_initial_seconds =
      scavengeInfo[1].duration_initial_seconds;
  } else {
    duration_factor =
      window.ScavengeScreen.village.options[1].base.duration_factor;
    duration_exponent =
      window.ScavengeScreen.village.options[1].base.duration_exponent;
    duration_initial_seconds =
      window.ScavengeScreen.village.options[1].base
        .duration_initial_seconds;
  }
...
function inputMemory() {
  ...
  if (has_archer) {
    ...
    document.getElementById("max_archer").value =
      parseInt(max_archer);
  }
  ...
  if (has_archer) {
    ...
    document.getElementById("max_marcher").value =
      parseInt(max_marcher);
  }
...
function calculateHaul(units) {
  var unitHauls = $.map(units, function (obj, key) {
    return obj * haulsPerUnit[key];
  });
  var totalHaul = 0;
  $.each(unitHauls, function () {
    totalHaul += this || 0;
  });
  return totalHaul;
}
...
function duration(x) {
  return (
    ((100 * x * x) ** duration_exponent + duration_initial_seconds) *
    duration_factor
  );
}
...
function getAvailableScavanges() {
  ...
  availableScavanges = $.grep(
    availableScavanges,
    function (obj, index) {
      return (
        $(
          "[class*='free_send_button']",
          $($("[class^='scavenge-option']")[obj])
        ).length > 0
      );
    }
  );
...
function get_optimal_factors(units, availableScavanges) {
  ...
  myconsolelog(availableScavanges);
  var arrayfactor = $.map(availableScavanges, function (obj, key) {
    return lootFactors[key];
  });
  ...
  worker.postMessage(
    JSON.stringify({
      ...
      otherStuff: [units, availableScavanges],
    })
  ); // Start the worker.
}
...
function optimization_callBack(
  optimization,
  units,
  availableScavanges
) {
  ...
  let btn = $(
    "[class*='free_send_button']",
    $($("[class^='scavenge-option']")[scavangeType])
  )[0];
  console.log(btn);
  ...
  var time = hours * 3600;
  var maxhaul =
    ((time / duration_factor - duration_initial_seconds) **
      (1 / duration_exponent) /
      100) **
    (1 / 2);
  ...
  if (predHaul > maxhaul)
    $.each(units, function (key, val) {
      units[key] = (maxhaul / predHaul) * val;
    });
...
function successfunc(data) {
  ...
  else unitNamesArray = ["spear", "sword", "axe", "light", "heavy"];
  ...
  $.each(unitNamesArray, function (index, val) {
    scavanging_units[val] = scavTable
      ? parseInt(
          $(
            "[class^='unit-item unit-item-" + val + "']",
            scavTable
          ).last()[0].innerText
        )
      : 0;
  ...
  $.each(availableUnits, function (key, obj) {
    ...
    if (max_units[key] < obj + scavanging_units[key]) {
      unitsToUse[key] = Math.max(
        max_units[key] - scavanging_units[key],
        0
      );
    } else unitsToUse[key] = obj;
  });
  myconsolelog("unitsToUse");
  myconsolelog(unitsToUse);
