...
if (localStorage.getItem("troopTypeEnabled") == null) {
  ...
  for (var i = 0; i < worldUnits.length; i++) {
    if (
      worldUnits[i] != "militia" &&
      worldUnits[i] != "snob" &&
      worldUnits[i] != "ram" &&
      worldUnits[i] != "catapult" &&
      worldUnits[i] != "spy" &&
      worldUnits[i] != "knight"
    ) {
  ...
  localStorage.setItem(
    "troopTypeEnabled",
    JSON.stringify(troopTypeEnabled)
  );
} else {
  console.log("Getting which troop types are enabled from storage");
  var troopTypeEnabled = JSON.parse(
    localStorage.getItem("troopTypeEnabled")
  );
}
...
if (localStorage.getItem("categoryEnabled") == null) {
  ...
  localStorage.setItem(
    "categoryEnabled",
    JSON.stringify(categoryEnabled)
  );
} else {
  console.log(
    "Getting which category types are enabled from storage"
  );
  var categoryEnabled = JSON.parse(
    localStorage.getItem("categoryEnabled")
  );
}
...
if (localStorage.getItem("prioritiseHighCat") == null) {
  ...
  localStorage.setItem(
    "prioritiseHighCat",
    JSON.stringify(prioritiseHighCat)
  );
} else {
  console.log("Getting prioritiseHighCat from storage");
  var prioritiseHighCat = JSON.parse(
    localStorage.getItem("prioritiseHighCat")
  );
}
...
var categoryNames = JSON.parse(
  "[" +
    $.find(
      'script:contains("ScavengeMassScreen")'
    )[0].innerHTML.match(/\{.*\:\{.*\:.*\}\}/g) +
    "]"
)[0];
      ...
      () => {
        ...
        if (count == scavengeInfo.length) {
          ...
          for (var k = 0; k < squad_requests.length; k++) {
            ...
            squads_premium[groupNumber].push(
              squad_requests_premium[k]
            );
          }
          ...
          for (var s = 0; s < Object.keys(squads).length; s++) {
            ...
            htmlWithLaunchButtons += `<tr id="sendRow${s}" style="text-align:center; width:auto; background-color:${backgroundColor}"><td style="text-align:center; width:auto; background-color:${backgroundColor}"><center><input type="button"  class="btn btnSophie" id="sendMass" onclick="sendGroup(${s},false)" value="${
              langShinko[8]
            }${
              s + 1
            }"></center></td><td style="text-align:center; width:auto; background-color:${backgroundColor}"><center><input type="button"  class="btn btn-pp btn-send-premium" id="sendMassPremium" onclick="sendGroup(${s},true)" value="${
              langShinko[8]
            }${
              s + 1
            } WITH PREMIUM" style="display:none"></center></td></tr>`;
          }
...
<button class="btn" id = "x" onclick="closeWindow('massScavengeSophie')">
  ...
  langShinko[3]
        ...
        <tr>
            ...
            <td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;"><input type="date" id="offDay" name="offDay" value="${setDayToField(
  runTimes.off
)}"><input type="time" id="offTime" name="offTime" value="${setTimeToField(
  runTimes.off
)}"></td>
            <td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;"><input type="date" id="defDay" name="defDay" value="${setDayToField(
  runTimes.def
)}"><input type="time" id="defTime" name="defTime" value="${setTimeToField(
  runTimes.def
)}"></td>
        </tr>
        <tr>
            ...
            <td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;"><input type="text" class="runTime_off" style="background-color:${backgroundColor};color:${titleColor};" value="${
  runTimes["off"]
}" onclick="this.select();"></td>
            <td style="text-align:center; width:auto; background-color:${backgroundColor}; padding:5px;"><input type="text" class="runTime_def" style="background-color:${backgroundColor};color:${titleColor};" value="${
  runTimes["def"]
}" onclick="this.select();"></td>
        </tr>
        <tr>
    ...
    <hr>
    <center><input type="button" class="btn btnSophie" id="sendMass" onclick="readyToSend()" value="${
      langShinko[5]
    }"></center>
    <hr>
...
$(".runTime_off")[0].addEventListener(
  "input",
  function () {
    updateTimers();
  },
...
$(".runTime_def")[0].addEventListener(
  "input",
  function () {
    updateTimers();
  },
...
$("#timeSelectorDate")[0].addEventListener(
  "input",
  function () {
    selectType("Date");
    updateTimers();
...
$("#timeSelectorHours")[0].addEventListener(
  "input",
  function () {
    selectType("Hours");
    updateTimers();
...
for (var i = 0; i < sendOrder.length; i++) {
  $("#imgRow").eq(0)
    .append(`<td align="center" style="background-color:${backgroundColor}">
    <table class="vis" border="1" style="width: 100%">
    <thead>
    </thead>
    <tbody>    
        <tr>
            <td style=" text-align:center;background-color:${headerColor};padding: 5px;"><img src="https://dsen.innogamescdn.com/asset/cf2959e7/graphic/unit/unit_${
    sendOrder[i]
  }.png" title="${sendOrder[i]}" alt="" class=""></td>
        </tr>
        <tr>
            <td align="center" style="background-color:${backgroundColor};padding: 5px;"><input type="checkbox" ID="${
    sendOrder[i]
  }" name="${sendOrder[i]}"></td>
        </tr>
        <tr>
        ...
        <tr>
            <td align="center" style="background-color:${backgroundColor};padding: 5px;"><input type="text" ID="${
    sendOrder[i]
  }Backup" name="${sendOrder[i]}" value="${
    keepHome[sendOrder[i]]
  }" size="5"></td>
        </tr>
        </tbody>  
    </table>
...
function readyToSend() {
  ...
  if (
    $("#settingPriorityPriority")[0].checked == false &&
    $("#settingPriorityBalanced")[0].checked == false
  ) {
    // no setting chosen
    alert(
      "You have not chosen how you want to split your troops! Choose either prioritising higher categories till chosen runtime, or balanced spread over all categories!"
    );
    throw Error("didn't choose type");
  }

  if (
    $("#category1").is(":checked") == false &&
    $("#category2").is(":checked") == false &&
    $("#category3").is(":checked") == false &&
    $("#category4").is(":checked") == false
  ) {
    // no category chosen
  ...
  console.log(sendOrder);
  for (var i = 0; i < sendOrder.length; i++) {
    troopTypeEnabled[sendOrder[i]] = $(
      `:checkbox#${sendOrder[i]}`
    ).is(":checked");
  }
  for (var i = 0; i < sendOrder.length; i++) {
  ...
  if ($("#timeSelectorDate")[0].checked == true) {
    localStorage.setItem("timeElement", "Date");
    time.off = Date.parse(
      $("#offDay").val().replace(/-/g, "/") +
        " " +
        $("#offTime").val()
    );
    time.def = Date.parse(
      $("#defDay").val().replace(/-/g, "/") +
        " " +
        $("#defTime").val()
    );
...
function calculateHaulCategories(data) {
  ...
  if (data.has_rally_point == true) {
    ...
    for (key in troopTypeEnabled) {
      if (troopTypeEnabled[key] == true) {
        if (data.unit_counts_home[key] - keepHome[key] > 0) {
          troopsAllowed[key] =
            data.unit_counts_home[key] - keepHome[key];
        } else {
          troopsAllowed[key] = 0;
        }
      }
    ...
    for (key in troopsAllowed) {
      if (key == "spear")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 25);
      if (key == "sword")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 15);
      if (key == "axe")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 10);
      if (key == "archer")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 10);
      if (key == "light")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 80);
      if (key == "marcher")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 50);
      if (key == "heavy")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 50);
      if (key == "knight")
        totalLoot +=
          troopsAllowed[key] * (data.unit_carry_factor * 100);
    }
    ...
    if (
      data.options[1].is_locked == true ||
      data.options[1].scavenging_squad != null
    ) {
    ...
    if (
      data.options[2].is_locked == true ||
      data.options[2].scavenging_squad != null
    ) {
    ...
    if (
      data.options[3].is_locked == true ||
      data.options[3].scavenging_squad != null
    ) {
      haulCategoryRate[3] = 0;
    } else {
      haulCategoryRate[3] = haul / 0.5;
    }
    if (
      data.options[4].is_locked == true ||
      data.options[4].scavenging_squad != null
    ) {
    ...
    for (var k = 0; k < Object.keys(unitsReadyForSend).length; k++) {
      candidate_squad = {
        unit_counts: unitsReadyForSend[k],
...
function calculateUnitsPerVillage(troopsAllowed) {
  ...
  if (totalLoot > totalHaul) {
    ...
    if (version != "old") {
      ...
      for (var j = 0; j < 4; j++) {
        for (key in troopsAllowed) {
          unitsReadyForSend[j][key] = Math.floor(
            haulCategoryRate[j + 1] * (troopsAllowed[key] / totalLoot)
          );
        }
      }
    }
    ...
    if (prioritiseHighCat != true && troopNumber > 130) {
      for (var j = 0; j < 4; j++) {
        ...
        for (key in troopsAllowed) {
          unitsReadyForSend[j][key] = Math.floor(
            (totalLoot / totalHaul) *
              haulCategoryRate[j + 1] *
              (troopsAllowed[key] / totalLoot)
          );
        }
      }
    } else {
...
function updateTimers() {
  if ($("#timeSelectorDate")[0].checked == true) {
    console.log("datebox");
    $("#offDisplay")[0].innerText = fancyTimeFormat(
      (Date.parse(
        $("#offDay").val().replace(/-/g, "/") +
          " " +
          $("#offTime").val()
    ...
    $("#defDisplay")[0].innerText = fancyTimeFormat(
      (Date.parse(
        $("#defDay").val().replace(/-/g, "/") +
          " " +
          $("#defTime").val()
    ...
    console.log("Textbox ");
    $("#offDisplay")[0].innerText = fancyTimeFormat(
      $(".runTime_off").val() * 3600
    );
    $("#defDisplay")[0].innerText = fancyTimeFormat(
      $(".runTime_def").val() * 3600
    );
  }
}
